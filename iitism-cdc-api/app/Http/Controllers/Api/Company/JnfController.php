...
class JnfController extends Controller
{
    public function create(Request $request)
    {
        ...
        $jnf = JobNotificationForm::create([
    ...
    public function show($id, Request $request)
    {
        ...
        $contacts = \DB::table('jnf_contacts')->where('jnf_id', $jnf->id)->first();
        $jnfData = $jnf->toArray();
        $jnfData['contacts'] = $contacts ? json_decode($contacts->data, true) : null;
    ...
    {
        $jnf = JobNotificationForm::where('id', $id)
            ->where('company_id', $request->user()->company->id)
            ->firstOrFail();
        ...
        // Strip non-fillable or nested relationship keys if required, assuming model casts handle json mapping natively.
        switch ($section) {
            case 'season':
                $jnf->update(['season' => $data['season'] ?? '2024-25']);
                break;
            case 'job_profile':
                $mappedData = [
                ...
                JnfJobProfile::updateOrCreate(['jnf_id' => $jnf->id], $mappedData);
                break;
            case 'eligibility':
                $genderMap = [
                    'all genders' => 'all',
                ...
                JnfEligibility::updateOrCreate(['jnf_id' => $jnf->id], $mappedData);
                break;
            case 'salary':
                $mappedData = [
                    'currency' => $data['currency'] ?? 'INR',
                    'salary_same_for_all' => (bool)($data['same_for_all'] ?? true),
                    'salary_data' => $data['salaries'] ?? [],
                    'additional_components' => [
                        'global' => $data['additional_global'] ?? null,
                        'per_course' => $data['additional_per_course'] ?? null,
                        'same_additional_for_all' => (bool)($data['same_additional_for_all'] ?? true),
                    ],
                ];
                JnfSalary::updateOrCreate(['jnf_id' => $jnf->id], $mappedData);
                break;
            case 'selection_process':
                ...
                JnfSelectionProcess::updateOrCreate(['jnf_id' => $jnf->id], $mappedData);
                break;
            case 'declaration':
                $mappedData = [
                ...
                JnfDeclaration::updateOrCreate(['jnf_id' => $jnf->id], $mappedData);
                break;
    case 'contacts':
    ...
    {
        $jnf = JobNotificationForm::with(['jobProfile', 'eligibility', 'salary', 'selectionProcess', 'declaration'])
            ->where('id', $id)
            ->where('company_id', $request->user()->company->id)
            ->firstOrFail();

        if ($jnf->status !== 'draft') {
            return response()->json(['success' => false, 'message' => 'Form is already submitted or under review'], 400);
        }
        ...
        // 1. Contacts Check
        $contactsData = \DB::table('jnf_contacts')->where('jnf_id', $jnf->id)->first();
        if (!$contactsData) {
            $errors[] = 'Contact details are missing.';
        }

        // 2. Job Profile
        if (!$jnf->jobProfile || empty($jnf->jobProfile->job_title) || empty($jnf->jobProfile->place_of_posting) || empty($jnf->jobProfile->expected_hires)) {
            $errors[] = 'Job Profile is incomplete.';
        }

        // 3. Eligibility
        if (!$jnf->eligibility || is_null($jnf->eligibility->min_cgpa) || empty($jnf->eligibility->programmes) || count($jnf->eligibility->programmes) == 0) {
            $errors[] = 'Eligibility details are incomplete or no courses selected.';
        }

        // 4. Salary
        if (!$jnf->salary || empty($jnf->salary->salary_data) || count($jnf->salary->salary_data) == 0) {
            $errors[] = 'Salary details are incomplete.';
        } else {
            $salaries = collect($jnf->salary->salary_data);
            $hasCtc = $salaries->contains(function ($s) {
                return !empty($s['ctc']) && $s['ctc'] > 0;
            });
            if (!$hasCtc) {
                $errors[] = 'Salary details must have CTC defined for at least one programme.';
            }
        }

        // 5. Selection Process
        if (!$jnf->selectionProcess || empty($jnf->selectionProcess->stages)) {
            $errors[] = 'Selection Process is missing.';
        } else {
            $stages = $jnf->selectionProcess->stages;
            $hasStage = false;
        ...
        // 6. Declaration
        if (!$jnf->declaration || empty($jnf->declaration->signatory_name) || empty($jnf->declaration->typed_signature)) {
        ...
        if (count($errors) > 0) {
            return response()->json([
                'success' => false, 
                'message' => 'Cannot submit JNF. Missing fields: ' . implode(" ", $errors)
            ], 400);
        }
        ...
        if (class_exists(FormSubmittedConfirmation::class)) {
            Mail::to($request->user()->email)->queue(new FormSubmittedConfirmation($jnf));
        }

        if (class_exists(AdminFormSubmittedAlert::class)) {
            Mail::to('admin@iitism.ac.in')->queue(new AdminFormSubmittedAlert($jnf));
        }
